...
def team_draft_interleaving(list_a, list_b):
    ''' Team draft interleaving is performed by throwing a coin. If its heads
        we put in the interleaved list the first element of list A that is not
        already in, if its tails we put in the interleaved list the first
        element of list B that is not already in.

        @Input: two lists of tuples, each containing 3 documents. Each document
        has an arbitrary document ID to distinguish from another on index 1, and
        some information, such as relevance score, on index 0.

        @Output: an interleaved list of tuples of length 3. Each tuple contains
        the information from the input and a 0 or 1 depending of which list it
        came from.
    '''
    ...
    while counter < 3:
        ...
        if(coin_toss > 0.5):
            put_first_available_url_in_interleaved(
                list_a, 0, interleaved_list, unique_doc_list)
            counter += 1
            ...
            put_first_available_url_in_interleaved(
                list_b, 1, interleaved_list, unique_doc_list)
            counter += 1
        else:
            put_first_available_url_in_interleaved(
                list_b, 1, interleaved_list, unique_doc_list)
...
def put_first_available_url_in_interleaved(a_list, index_list, interleaved_list, unique_doc_list):
    ''' Helper method that creates a tuple with and integer (document ID) and
        a 0 or 1 which indicates from which list it came from and adds it in the
        interleaved list.

        @Input:
            - a_list: a list of tuples of length 3, where each tuple has
                some data on index 0 and a doc_id on index 1.
            - index_list: a 0 or 1 indicator of the list.
            - interleaved_list: passed by reference interleaved list with all tuples of the rankings
            - unique_doc_list: passed by reference list of processed unique docs
    '''
...
# %%
# TEST
test_pairs = calculate_Dmeasures(rankings)
...
def calc_sample_size_for_bins(interleave_fn=team_draft_interleaving, model=model_PBM, rankings=rankings):
    ...
    table = pd.DataFrame(index=bin_vals, columns=[
...
def calc_sample_size_for_bin(binned_el, interleave_fn, model):
    ...
    for pair in binned_el:
        ...
        proportion_E_win = simulate_experiment(
            pairE, pairP, model, interleave_fn)
